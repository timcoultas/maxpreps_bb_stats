...
This project answers a simple question every coach asks in the offseason: *"What does next year's team look like?"*

We built a complete projection pipeline that takes historical player statistics from MaxPreps, applies development curves to returning players, fills roster gaps with realistic replacement-level talent, and simulates an entire season's worth of games to produce win probabilities and expected records.

The system was designed for Colorado 5A baseball, specifically to project Rocky Mountain High School's 2026 season. However, the methodology is portable to any high school program with sufficient historical data.

**What this is:** A data-driven scouting report generator. It tells you which returning juniors are poised for breakout seasons, which roster spots need reinforcements, and which games on the schedule are likely wins, toss-ups, or uphill battles.
...
The pipeline produces four key deliverables:

### 1. Development Multipliers (`development_multipliers.csv`)
Year-over-year performance ratios showing how players typically improve (or regress) as they advance through high school. For example, the median Sophomore→Junior sees a 1.15x increase in hits. These multipliers drive all downstream projections.

### 2. Generic Player Profiles (`generic_players.csv`)
Statistical profiles representing "replacement level" talent at various percentiles. When a team loses seniors and needs to backfill roster spots, we don't assume zeros—we assume realistic JV call-up production based on historical sophomore performance.

### 3. Projected Rosters (`2026_roster_prediction.csv`)
Complete roster projections for every team in the dataset. Each returning player has their stats projected forward using development multipliers. Roster gaps are filled with tiered generic players (elite programs get 50th percentile replacements; rebuilding programs get 30th percentile).

### 4. Season Simulation (`rocky_mountain_monte_carlo.csv`)
Game-by-game win probabilities generated by running 1,000 Monte Carlo simulations per matchup. Includes projected scores, confidence labels ("Lock", "Solid", "Toss-up"), and a season summary with floor/ceiling win totals.
...
> **⚠️ STUB SECTION — TO BE COMPLETED**
>
> This section should document:
> 
> 1. **Manual Download Process**
>    - Navigate to MaxPreps team stats page
>    - Save HTML files to appropriate `data/raw/{team}/{period}/` folder
>    - Naming conventions for files
>
> 2. **Automated Scraping (if implemented)**
>    - Script location and usage
>    - Rate limiting and ethical considerations
>    - MaxPreps Terms of Service compliance
>
> 3. **Required Pages per Team**
>    - Batting stats page
>    - Pitching stats page  
>    - Fielding stats page (optional)
>    - Which seasons to collect (recommendation: 3+ years of history)
>
> 4. **Schedule File Format**
>    - CSV structure: `Date, Home, Away` (or `Date, Opponent, Location`)
>    - Where to obtain schedule data
>    - Example file provided in `data/input/`
